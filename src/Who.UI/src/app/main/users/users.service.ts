...
export class UsersService {
    ...
    //#region  Users
    SubscribeUserEvents() {
        this.messageService.Stream<DataEvent<MUserListDto>>("WhoUser.Subscribe").pipe(
            tap(item => {
                console.log(item)
                switch (item.Action) {
                    case "Created": {
    ...
    ReLoadUsers() {
        this.http.get<Array<MUserListDto>>(`/api/admin/users`, { headers: this.getHeaders() }).pipe(
            tap(users => this.usersStore.set(users))
        ).subscribe();
    }
    
    GetUser(id: string) {
        return this.http.get<MUserDto>(`/api/admin/users/${id}`, { headers: this.getHeaders() })
    }

    CreateUser(createUserModel: MUserDto) {
        return this.http.post<string>(`/api/admin/users`, createUserModel, { headers: this.getHeaders() });
    }

    UpdateUser(createUserModel: MUserDto) {
        return this.http.put(`/api/admin/users`, createUserModel, { headers: this.getHeaders() });
    }
    ...
    DeleteUser(...ids: string[]) {
        ...
        return this.http.request("delete", `/api/admin/users`, options);
    }


    SetPassword(userId: string, dto: SetPasswordDto) {
        return this.http.post(`/api/admin/users/${userId}/password`, dto, { headers: this.getHeaders() });
    }

    ClearPassword(userId: string) {
        return this.http.delete(`/api/admin/users/${userId}/password`, { headers: this.getHeaders() });
    }
