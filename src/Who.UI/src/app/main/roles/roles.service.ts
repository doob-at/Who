...
export class RolesService {
    ...
    SubscribeRoleEvents() {
        console.log("WhoRole.Subscribe")
        this.messageService.Stream<DataEvent<MRoleDto>>("WhoRole.Subscribe").pipe(
            tap(item => {
                console.log(item)
                switch (item.Action) {
                    case "Created": {
    ...
    ReLoadRoles() {
        this.http.get<Array<MRoleDto>>(`/api/admin/roles`, { headers: this.getHeaders() }).pipe(
            tap(users => this.roleStore.set(users))
        ).subscribe();
    }
    
    GetRole(id: string) {
        return this.http.get<MRoleDto>(`/api/admin/roles/${id}`, { headers: this.getHeaders() })
    }

    CreateRole(roleModel: MRoleDto) {
        return this.http.post(`/api/admin/roles`, roleModel, { headers: this.getHeaders() });
    }

    UpdateRole(roleModel: MRoleDto) {
        return this.http.put(`/api/admin/roles`, roleModel, { headers: this.getHeaders() });
    }
    ...
    DeleteRole(...ids: string[]) {
        ...
        return this.http.request("delete", `/api/admin/roles`, options);
    }
    //#endregion
