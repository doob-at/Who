...
export class AuthProviderService {
    ...
    ReLoadAuthProviders() {
        this.http.get<Array<AuthProviderDto>>(`api/admin/auth-provider`, { headers: this.getHeaders() }).pipe(
            tap(users => this.AuthProvidersStore.set(users))
        ).subscribe();
    }

    GetAuthProvider(id: string) {
        return this.http.get<AuthProviderDto>(`api/admin/auth-provider/${id}`, { headers: this.getHeaders() })
    }

    CreateAuthProvider(dtoModel: AuthProviderDto) {
        return this.http.post(`api/admin/auth-provider`, dtoModel, { headers: this.getHeaders() });
    }

    UpdateAuthProvider(dtoModel: AuthProviderDto) {
        return this.http.put(`api/admin/auth-provider`, dtoModel, { headers: this.getHeaders() });
    }
    ...
    DeleteAuthProvider(...ids: string[]) {
        ...
        return this.http.request("delete", `api/admin/auth-provider`, options);
    }
